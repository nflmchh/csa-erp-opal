...
<div class="space-y-4">
    ...
    <form method="GET" class="bg-white rounded-xl border border-gray-200 p-4 flex flex-wrap gap-3 items-end">
        ...
        <div>
            ...
            <select name="warehouse_id"
                ...
                <option value="{{ $w->id }}" {{ ($warehouseId ?? request('warehouse_id')) == $w->id ? 'selected' : '' }}>{{ $w->name }}</option>
                @endforeach
            </select>
        </div>
        @endif
        ...
        <div>
            ...
            <select name="store_id"
                ...
                @foreach($stores as $s)
                <option value="{{ $s->id }}" {{ ($storeId ?? request('store_id')) == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                @endforeach
            </select>
        </div>
        @endif
    ...
    <div class="flex items-center gap-2 flex-wrap">
        ...
        <a href="{{ route('exports.shipment.pdf', request()->query()) }}" target="_blank"
        ...
        <a href="{{ route('exports.shipment.excel', request()->query()) }}"
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right text-xs font-semibold text-indigo-600">{{ $shipment->totalQtySent() }}</td>
